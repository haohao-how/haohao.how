...
export class ExpoSQLiteKVStore implements ExperimentalKVStore {
  ...
  async read(): Promise<ExperimentalKVRead> {
    ...
    return new ReplicacheGenericSQLiteReadImpl(db, () => {
      this.release(db);
    });
  }
  ...
  async write(): Promise<ExperimentalKVWrite> {
    ...
    const db = await this.open();
    await db.execAsync("BEGIN");
    return new ReplicacheGenericSQLiteWriteImpl(db, () => {
      this.release(db);
    });
  }
...
export class ReplicacheGenericSQLiteReadImpl implements ExperimentalKVRead {
